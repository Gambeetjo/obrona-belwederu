...
(async () => {
  ...
  try {
    ...
    const results=await page.evaluate(() => {
      ...
      fresh();wave=14;Math.random=()=>0;showLevelUpScreen();check('Nerko unavailable before wave 15',!document.querySelector('.legendary'));wave=15;showLevelUpScreen();check('Nerko offered on successful roll',!!document.querySelector('.legendary'));Math.random=()=>0.25;showLevelUpScreen();check('Nerko roll excludes 25 percent boundary',!document.querySelector('.legendary'));
      fresh();spawnNerko();const ranged=enemy('thrower',nerko.x+13,nerko.z);const second=enemy('thrower',nerko.x+14,nerko.z);const melee=enemy('normal',nerko.x+8,nerko.z);const oldHp=ranged.hp;updateNerko(0.01);check('Nerko bites only one ranged enemy at half damage',ranged.hp===oldHp-player.dmg*0.5&&second.hp===second.maxHp&&melee.hp===melee.maxHp);const after=ranged.hp;updateNerko(0.1);check('Nerko bite cooldown',ranged.hp===after);wave=15;Math.random=()=>0;showLevelUpScreen();check('Owned Nerko not offered again',!document.querySelector('.legendary'));initGame();check('Restart clears Nerko',nerko===null&&nerkoUpgrade.level===0);
      fresh();spawnNerko();const fallback=enemy('normal',nerko.x+13,nerko.z);updateNerko(0.01);check('Nerko attacks ordinary enemies when no ranged remain',fallback.hp<fallback.maxHp);const priority=enemy('thrower',nerko.x+100,nerko.z);updateNerko(0.01);check('Nerko switches to ranged priority',nerko.target===priority);
      fresh();wave=16;spawnOpeningEnemies(16);check('Wave 16 guarantees two bandits',enemies.filter(e=>e.type==='bandit').length>=2);
      fresh();const tougher=enemy('mexicano',200,200);check('Boss health speed and stagger increased',tougher.maxHp===1620&&tougher.speed===60&&tougher.maxStagger===375);tougher.isStaggered=true;tougher.staggerTimer=0.01;update(0.02);check('Boss gains stagger recovery protection',tougher.staggerImmuneUntil>gameTime);player.x=tougher.x-60;player.z=tougher.z;player.attackTimer=0;performPlayerAttack();check('Recovery protection blocks stagger but allows damage',tougher.stagger===0&&tougher.hp<tougher.maxHp);
    ...
    await page.evaluate(()=>{initGame();pendingRewards=1;showLevelUpScreen();});
    const card=page.locator('.bonus-card').first();
    await card.click(); assert.equal(await page.evaluate(()=>gameState),'levelup');
    await card.hover(); await page.mouse.down(); await page.waitForTimeout(400); await page.mouse.up(); await page.waitForTimeout(700);
    assert.equal(await page.evaluate(()=>gameState),'levelup');
    await card.hover(); await page.mouse.down(); await page.waitForTimeout(1100); await page.mouse.up();
    assert.equal(await page.evaluate(()=>gameState),'playing');
    console.log('PASS upgrades reject clicks and short holds; accept one-second hold');
