...
(async () => {
  ...
  try {
    ...
    await card.hover(); await page.mouse.down(); await page.waitForTimeout(700); await page.mouse.up();
    assert.equal(await page.evaluate(()=>gameState),'playing');
    console.log('PASS upgrades reject clicks and short holds; accept 0.6-second hold');
